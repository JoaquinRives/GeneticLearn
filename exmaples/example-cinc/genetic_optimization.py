...
if __name__ == '__main__':
    ...
    chr1 = Chromosome(genes=(gen1, gen2, gen3, gen4, gen5, gen6, gen7, gen8, gen9), name='chr1', mutation='uniform',
                      recombination='poisson', r_prob=0.5, m_prob=0.1,
                      r_lambda=1)
    ...
    def fitness_func(X, y, param_grid):
        ...
        # Compute F_beta measure and the generalization of the Jaccard index
        accuracy, f_measure, Fbeta_measure, Gbeta_measure = compute_beta_score(y, final_prediction, 2, num_classes)

        # compute AUROC and AUPRC
        auroc, auprc = compute_auc(labels, prob_optimized, num_classes)
    ...
    history = population.evolve(n_gen=5, selection_method='elitism')
    #
    ...
    # change this for model.predict() (validation), the rest is all right # TODO
    # labels, output, output_probabilities = evaluate_12ECG_score('test_1', 'output_old')

    labels = test_labels
    output, output_probabilities = model.predict(X_test)

    optimized_prob = winner['w1', ...] * output_probabilities
    optimized_labels = np.where(optimized_prob > 0.5, 1, 0)

    num_classes = 9

    # comparison
    accuracy, f_measure, Fbeta_measure, Gbeta_measure = compute_beta_score(labels, output, 2, num_classes)
    accuracy_opt, f_measure_opt, Fbeta_measure_opt, Gbeta_measure_opt = compute_beta_score(labels, optimized_labels, 2, num_classes)

    # compute AUROC and AUPRC
    auroc, auprc = compute_auc(labels, output_probabilities, num_classes)
    auroc_opt, auprc_opt = compute_auc(labels, optimized_prob, num_classes)
